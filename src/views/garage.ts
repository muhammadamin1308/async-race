...
import { startCarEngine, driveCar, stopCarEngine } from '@/api/engine-api';
import { createCarHandler, updateCarHandler, deleteCarHandler } from "@/components/api-handler";
export default function renderGarageView(container: HTMLElement): void {
  container.innerHTML = `
  <div class="top-panel">
    <div class="form-row">
      <form id="create-form" class="form-inline">
        <input class="form-input" type="text" id="create-name" placeholder="Car name" required />
        <input class="color-input" type="color" id="create-color" value="#00ff00" />
        <button class="btn-effect" type="submit">CREATE</button>
      </form>

      <form id="update-form" class="form-inline">
        <input class="form-input" type="text" id="update-name" placeholder="New name" />
        <input class="color-input" type="color" id="update-color" value="#00ff00" />
        <button class="btn-effect" type="submit">UPDATE</button>
      </form>
    </div>

    <div class="action-buttons">
      <button class="btn-effect" id="race">RACE</button>
      <button class="btn-effect" id="reset">RESET</button>
      <button class="btn-effect" id="generate-cars">GENERATE CARS</button>
    </div>
  </div>
...
async function paginationHandler(): Promise<void> {
  ...
  document.querySelectorAll('.start-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      const car = document.querySelector(`.car[data-id="${id}"]`) as HTMLElement;
      const track = document.querySelector(`.car-track[data-id="${id}"]`) as HTMLDivElement;
      const { velocity } = await startCarEngine(id);
      const distance = track.clientWidth - car.clientWidth - 80; // 20px for the finish line
  ...
  // Reset all cars position
  document.getElementById('reset')?.addEventListener('click', async () => {
    const carIcons = document.querySelectorAll('.car-icon') as NodeListOf<HTMLElement>;
    carIcons.forEach((car) => {
      resetCarPosition(car as HTMLElement);
    });
  });
  
  function resetCarPosition(car: HTMLElement): void {
  ...
  document.getElementById('race')?.addEventListener('click', async () => {
    ...
    const racePromises = Array.from(startButtons).map(async (btn) => {
      const id = Number(btn.dataset.id);
      const car = document.querySelector(`.car[data-id="${id}"]`) as HTMLElement;
      const track = document.querySelector(`.car-track[data-id="${id}"]`) as HTMLDivElement;
      const { velocity } = await startCarEngine(id);
      const distance = track.clientWidth - car.clientWidth - 80;
      const time = distance / velocity;
      ...
      return { id, time, success: driveResponse.success };
    });
    ...
    if (winner) {
      alert(`Car ${winner.id} wins the race in ${winner.time.toFixed(2)} seconds!`);
    } else {
...
export function handleSelectButton(): void {
